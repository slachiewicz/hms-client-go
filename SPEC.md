...
3. **Retry budget**: at most 3 attempts per RPC across endpoints by default (`WithMaxRetries`). Two distinct decisions apply:
   ...
   * `GetPartitionsSeq` and `GetTablesSeq` (§5.4, §5.5) are read-only but deliberately excluded from the idempotent-retry list above: each issues several RPCs (a names call, then one or more by-name chunks) while yielding results to the caller as they arrive, and a retry that re-ran the whole sequence from its first RPC would re-yield results the caller has already consumed. Once either call's own fn has started, any failure -- including one that would otherwise be retried as `ErrUnavailable` -- is returned immediately instead.
...
`GetTablesSeq` (1.0 addition) is `GetTables`' streaming, memory-bounded form: `GetAllTables` returns every table name in one round trip already (a name is cheap to hold in full), but `GetTables` still builds the whole `[]*Table` slice before returning it. `GetTablesSeq` instead calls `GetAllTables` once and then `get_table_objects_by_name_req` in chunks of `WithChunkSize`, yielding each `*Table` as its chunk arrives, so at most one chunk's worth of tables is ever held in memory regardless of how many the database has. Breaking out of the range loop early issues no further RPCs; on failure exactly one `(nil, err)` pair is yielded and the sequence ends. Every RPC this call issues runs inside one non-retried attempt (SPEC §4.2's read-retry-on-`ErrUnavailable` does not apply here): a retry would re-run the whole sequence from `GetAllTables` and re-yield tables already handed to the caller, which this call avoids by never retrying once it has started.
