...
| `get_database`, `get_all_databases`, `create_database`, `drop_database`, `alter_database`, `get_all_tables`, `create_table`, `alter_table`, `drop_table`, `get_partitions`, `get_partition_names`, `alter_partitions`, `drop_partition`, `drop_partitions_req`, `get_partitions_by_names`, `get_partitions_by_filter`, `get_partition_names_ps`, `get_config_value` | Y | Y | Y | Y |
...
* **Rule 2 (`get_table_req`, `get_table_objects_by_name_req`, `add_partitions_req`)**: No fallback. All three exist on every supported version and the legacy forms are absent from the Hive 4 IDL. `drop_partitions_req` likewise carries no fallback, for a related but distinct reason: it is declared, and behaves the same, in the 2.3.9 and 3.1.3 IDL as well as 4.2.1's, so unlike `alter_partitions_req`/`get_partitions_req` and their siblings it was never a Hive-4-only addition with a version gap to bridge.
...
* **Rule 5 (batching)**: `add_partitions_req`, `alter_partitions_req`/`alter_partitions`, `drop_partitions_req`, and `get_table_objects_by_name_req` are chunked (default 1000 items per request) on every version to bound request size. `add_partitions_req`, `alter_partitions_req`/`alter_partitions`, and `drop_partitions_req`'s batch size is governed by `WithPartitionBatchSize`, independent of `WithChunkSize`, which governs `get_table_objects_by_name_req` and `get_partitions_by_names_req`/`get_partitions_by_names` (Rule 6) instead — the two knobs do not share a value, so tuning one does not silently change the other's batching (§5.1). For `alter_partitions_req`, the `UNKNOWN_METHOD` fallback decision (Rule 3) is made at most once per call: it is cached per connection, keyed by method, so only the first batch that reaches a given connection ever attempts `alter_partitions_req` and observes the fallback; every later batch on that connection goes straight to `alter_partitions`.
...
* **Authentication**:
  * `NOSASL` / `NONE`: raw binary protocol, the default. Unless `WithoutUGI` is set or SASL auth is configured (`WithPlainAuth`/`WithKerberos`), the client calls `set_ugi(user, groups)` once per newly dialed connection, mirroring the Java `HiveMetaStoreClient`'s behavior under `hive.metastore.execute.setugi`: `user` is `WithUser`'s value when set, else the current OS user (the same default the HTTP transport's `x-actor-username` already uses), resolved once by `New` rather than per dial; `groups` comes from `WithUserGroups` (§5.1). This means `New` performs an RPC on every newly dialed binary connection, not merely the dial itself -- a bare listener that accepts a TCP connection but never speaks Thrift no longer models a live server for a test double; `WithoutUGI` restores dial-only behavior. The call is bounded by `WithConnectTimeout`, the same as the dial and any SASL/TLS handshake above, rather than by the per-call `WithTimeout` a request with no context deadline would otherwise fall back to: it is part of connection establishment, not a per-call RPC.
  ...
  * **TLS**: `WithTLS(cfg *tls.Config)` wraps the dialed socket in `tls.Client` and completes its handshake -- bound to `ctx`/`WithConnectTimeout` exactly as the SASL handshake is (see the context-propagation bullet above) -- before the SASL/binary protocol layers are attached, for a server configured with `metastore.use.SSL=true`. An empty `cfg.ServerName` defaults to the host of the endpoint being dialed (the behaviour of `tls.Dial`, which `tls.Client` alone lacks), on a clone of `cfg`, so a config holding only `RootCAs` verifies each endpoint of an HA list by its own name; a non-empty `ServerName` or `InsecureSkipVerify` is used as given. `ContextClient` still owns deadlines on the raw `net.Conn`, not the `tls.Conn`: `(*tls.Conn).SetDeadline` and its `Read`/`Write` counterparts delegate straight to the underlying conn, so a deadline set on the raw conn already bounds the TLS conn's I/O. See §5.1.
...
* **Connection setup timing**: `WithConnectTimeout` (§3.1, §5.1) has no effect here -- it bounds dialing, the TLS handshake, and the SASL handshake / `set_ugi` over `thrift://` only. Over `http(s)://`, connection setup (dialing, TLS) is governed by the `*http.Client`'s own `Transport` (a caller-supplied one via `WithHTTPClient`, or the default this package builds), and each request -- including the first, which establishes the connection -- is bounded by `WithTimeout` when the caller's `ctx` carries no deadline of its own, the same as every later call.
...
func WithConnectTimeout(d time.Duration) Option // thrift:// only: dial / TLS handshake / SASL handshake / set_ugi timeout; defaults to WithTimeout's value; over http(s):// connection setup is governed by WithHTTPClient / WithTimeout instead (§3.2)
...
`GetTablesSeq` (1.0 addition) is `GetTables`' streaming, memory-bounded form: `GetAllTables` returns every table name in one round trip already (a name is cheap to hold in full), but `GetTables` still builds the whole `[]*Table` slice before returning it. `GetTablesSeq` instead calls `GetAllTables` once and then `get_table_objects_by_name_req` in chunks of `WithChunkSize`, yielding each `*Table` as its chunk arrives, so at most one chunk's worth of tables is ever held in memory regardless of how many the database has. Breaking out of the range loop early issues no further RPCs; on failure exactly one `(nil, err)` pair is yielded and the sequence ends. The names call and each chunk fetch are separate RPCs, each its own pooled-connection acquire/release (§4.2 point 3): a chunk's connection is released as soon as that chunk's fetch completes, before any of its tables are yielded, so a caller that blocks between items, or that itself calls this `Client` from inside the range loop, never holds up the connection pool, and `WithRPCObserver` (§5.10) sees one `RPCInfo` per RPC rather than one covering the whole sequence.
...
`DropPartitions` (1.0 addition) removes the partitions of `tableName` in `dbName` identified by `partVals`, each in partition-key order (the same shape `DropPartition`'s own `partVals` takes, one entry per partition instead of one partition per call); `deleteData` and `ifExists` behave exactly as `DropPartitionsByNames` describes. It first calls `GetTable` to learn the table's partition keys, then builds each partition's name with `PartitionName` before delegating to `DropPartitionsByNames` — a caller that already has partition names should call `DropPartitionsByNames` directly rather than pay for the extra `GetTable` round trip.

`GetPartitionsSeq` (1.0 addition, G11) is `GetPartitions`' streaming, memory-bounded form. `GetPartitions(maxParts=-1)` returns every partition in one Thrift response, so an `iter.Seq2` wrapped around it would not actually bound memory: the whole response, and every converted `*Partition`, is already in memory before the first one is yielded. `GetPartitionsSeq` instead calls `GetPartitionNames` once (names are cheap to hold in full) and then `GetPartitionsByNames` in chunks of `WithChunkSize` (default 1000, the same knob and default `GetPartitionsByNames` itself chunks by), yielding each `*Partition` as its chunk arrives: at most one chunk's worth of partitions is held in memory at a time. It has no `maxParts` parameter -- a caller wanting fewer than all partitions stops ranging early, which issues no further RPCs, rather than passing a limit the server would apply before any of them are seen. Against a server lacking `get_partitions_by_names_req` (Hive 2.3 and 3.x), each chunk independently degrades to the legacy `get_partitions_by_names` RPC, exactly as `GetPartitionsByNames` describes, with the fallback decision cached per connection (§2.3 Rule 5). On failure, exactly one `(nil, err)` pair is yielded and the sequence ends.

The names call and each chunk fetch are separate RPCs, each its own pooled-connection acquire/release (§4.2 point 3), not one connection held for the whole sequence: a chunk's connection is released as soon as that chunk's fetch completes, before any of its partitions are yielded to the caller. A caller that blocks between items, or that itself calls this `Client` from inside the range loop body, therefore never holds up the connection pool the way it would if the sequence's own connection were still checked out; `WithRPCObserver` (§5.10) sees one `RPCInfo` per RPC -- `get_partition_names` once, then one per chunk, with the chunk's real wire name -- each `Duration` measuring only that RPC, never any part of the caller's own loop body. Each chunk's fetch is retried across endpoints on `ErrUnavailable` exactly as `GetPartitionsByNames` itself is (§4.2 point 3): safe because a chunk is yielded only after its own fetch fully completes, so a retry re-runs just that one chunk's fetch, which has not yet yielded anything from it.

`GetPartitions`, `GetPartitionsByNames`, `GetPartitionsByFilter`, and `GetPartitionsSeq` intern `Storage.Columns` within one call (1.0 addition, G11): partitions the call returns whose columns are equal (`Name`/`Type`/`Comment`) share a single `[]*FieldSchema` rather than each getting its own converted copy -- a real win when a table's partitions, as is typical, all share one schema. `GetPartitionsByNames` and `GetPartitionsSeq` share one such cache across every chunk they issue, not just within a chunk. See `Partition.Storage`'s doc comment for the resulting contract: the shared slice must be copied before it is mutated.

`PartitionName` (1.0 addition) builds a Hive-style partition name (`"key1=value1/key2=value2/..."`) from a table's partition keys and one partition's values, following Hive's own `Warehouse.makePartName`/`FileUtils.makePartName` rules exactly so the result matches what a real metastore computes for the same values: each key is lowercased before being escaped (as `Warehouse.makePartName` does), each key and value is escaped individually, and an empty key or value becomes `"__HIVE_DEFAULT_PARTITION__"`. Escaping follows `FileUtils.escapePathName`'s Unix branch (HMS paths are HDFS/object-store paths, not local Windows filesystem paths, so `Shell.WINDOWS`'s extra escapes for space, `<`, `>`, `|` do not apply): every ASCII control character (`0x00`-`0x1F`), `DEL` (`0x7F`), and the literal clash set `"`, `#`, `%`, `'`, `*`, `/`, `:`, `=`, `?`, `\`, `{`, `[`, `]`, `^` are percent-encoded as `%XX` (uppercase hex); notably, space (`0x20`) is *not* escaped. It returns an error if `len(keys) != len(values)`.
...
func (c *Client) GetTableColumnStatisticsForEngine(ctx context.Context, db, tbl, engine string, columns []string, opts ...CatalogOption) ([]ColumnStatistics, error) // 1.0 addition
...
Hive 4's metastore stores column statistics per computing engine (`engine`, above): statistics Spark, Impala, or another engine wrote under its own engine name are a separate row from Hive's, even for the same column. `GetTableColumnStatistics` always requests the `"hive"` engine's statistics (`TableStatsRequest.Engine`'s IDL default, which this call never overrides). `GetTableColumnStatisticsForEngine` (1.0 addition) requests the named engine's set instead, sending `engine` verbatim as `TableStatsRequest.Engine`; it is otherwise identical (one round trip, same `read` retry class per §4.2, same empty-`columns` short circuit, same catalog resolution). An empty `engine` returns `ErrInvalidOperation` without issuing the RPC, since the server would look up a literal empty engine name rather than fall back to the default. Because `engine` is a 4.x-only field, a 2.3 or 3.x server skips it and answers with its single engine-less set, so on those versions the two methods return the same result for any `engine`; the client does not probe for that (the catalog probe separates only 2.3 from 3+), so a caller that must know checks `ServerVersion` (§5.6). `hmstest` models exactly this: `SeedColumnStatsForEngine` seeds a named engine's set, served by name on `Hive40` and ignored in favour of the `"hive"` set on `Hive23`/`Hive31`.
...
func (c *Client) Heartbeat(ctx context.Context, txnID int64, lockID int64) error // either id may be 0 to omit it, not both
...
`OpenTransaction` wraps `open_txns` with `OpenTxnRequest{NumTxns: 1, User: user, Hostname: host}` and returns the single allocated `txn_ids[0]`. `Lock`, `CheckLock`, and `Unlock` wrap `lock`, `check_lock`, and `unlock` respectively; `Heartbeat` wraps `heartbeat` with `HeartbeatRequest{TxnId, LockId}` (either may be omitted by passing 0, matching the RPC's optional fields). Since 0 means "none", a negative transaction or lock id is a caller mistake and is rejected with `ErrInvalidOperation` before any RPC is issued; so is `Heartbeat(0, 0)`, which would send an empty request that names nothing to keep alive.
