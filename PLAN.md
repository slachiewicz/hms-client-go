...
- [x] `hmstest`: a `set_ugi` handler recording the `(user, groups)` it was called with, so a test can assert it fired exactly once per new connection and never over HTTP or under SASL PLAIN/Kerberos.
...
- [x] `hmstest`: handlers for the six RPCs above (the four legacy plus the two 4.x-only request variants), backed by the existing in-memory store; `GetPartitionsByFilter`'s fixture supports the `key = 'value'` subset of Hive's filter grammar, joined by `and`, rejecting anything else with `MetaException`.
...
- [x] `hmstest`: an append-only event log driven by the store's own mutations (create/drop/alter), plus handlers for the two RPCs.
...
- [x] `hmstest`: a `get_table_statistics_req` handler serving fixture stats for at least one column of each of the eight exposed types (bool/long/double/string/binary/decimal/date/timestamp).
...
- [x] Covered by the KDC-backed leg below and green on its first run (2026-09-10, run 34522568583): the AS and TGS exchanges against a real MIT KDC, keytab loading, and interoperability with Hive 4.2.1's JDK `SaslServer` acceptor, whole suite included. Whether that acceptor sends an AP_REP subkey or rotates wrap tokens is not observed by the test, only tolerated: both branches are unit-tested against the fake acceptor, and the real one is satisfied whichever it takes.
- [x] Integration matrix: `hive-4.2.1-kerberos`, on the stock image. `test/docker/kdc` builds an MIT KDC sidecar in the job (apt-only), which creates `EXAMPLE.COM`, adds `hive/hms.example.com` and `client`, and exports both keytabs to a directory the metastore container and the runner mount. The metastore gets `metastore.sasl.enabled`, principal, keytab and `java.security.krb5.conf` as `-D` flags, plus a `core-site.xml` with `hadoop.security.authentication=kerberos` through `HIVE_CUSTOM_CONF_DIR` (a directory holding only that file, since the entrypoint symlinks files individually): without it `UserGroupInformation`'s keytab login is a no-op and the acceptor has no credentials. The suite dials `thrift://hms.example.com:9083` (mapped in `/etc/hosts`) so the client derives the service principal; `KRB5_CONFIG` points the runner at the KDC's published TCP port with `udp_preference_limit = 1`. `TestKerberos` additionally asserts a NOSASL client and a principal missing from the keytab are refused.

### Slice 15: Observability & API stability
- [x] `options.go`: `WithLogger`, `WithRPCObserver` per SPEC §5.10. `client.go`: emit `RPCInfo` from the `call`/`read` retry loop (`do`); log connection dial/close, `MarkFailed`/`MarkHealthy` transitions, and recovery-probe sweeps through the configured logger. `MarkFailed`/`MarkHealthy` (`internal/ha`) report whether the call was a real transition, so a transition is logged at Info and a repeat is logged at Debug (or not at all), rather than once per attempt/success.
